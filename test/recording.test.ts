...
import { RecordingSession } from '../src/recorder/RecordingSession';
...
suite('Recording Session Tests', () => {
  ...
  test('Should handle terminal steps', async () => {
    ...
    assert.strictEqual((buildlog.steps[1] as any).outcome, 'success');
    assert.strictEqual((buildlog.steps[2] as any).outcome, 'failure');
  });
});
